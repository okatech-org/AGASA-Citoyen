...
const URGENCE_LEVELS = [
    ...
    { id: "critique", label: "Critique", emoji: "🔴", color: "#f43f5e" },
    { id: "importante", label: "Importante", emoji: "🟠", color: "#f59e0b" },
    { id: "moderee", label: "Modérée", emoji: "🟡", color: "#F9A825" },
    { id: "information", label: "Info", emoji: "🔵", color: "#3b82f6" },
];
    ...
    return (
        <div className="min-h-dvh bg-bg-muted pb-24">
            ...
            <div className="bg-white border-b border-border px-4 py-4">
                ...
                {/* Urgency Filters */}
                <div className="mb-2">
                    <p className="text-[10px] font-semibold text-text-muted uppercase tracking-wider mb-1.5">Niveau d&apos;urgence</p>
                ...
                {/* Scope Filters */}
                <div>
                    <p className="text-[10px] font-semibold text-text-muted uppercase tracking-wider mb-1.5">Portée géographique</p>
                ...
                <div className="mx-4 mt-3 glass-card p-4 border border-blue-200/60 animate-slide-down">
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                            <p className="text-sm font-bold text-text">Recevez une alerte immédiate</p>
                            <p className="text-xs text-text-muted mt-0.5">Si un produit dangereux est rappelé dans votre zone.</p>
                            <div className="flex gap-2 mt-3">
                                ...
                                <button onClick={handleDismissPush} className="px-4 py-2 text-text-muted text-xs font-medium hover:text-text hover:bg-bg-muted rounded-xl transition-colors">
                                    Plus tard
                                </button>
                            </div>
                        </div>
            ...
            <div className="px-4 py-3 space-y-3">
                {filteredAlertes.length === 0 ? (
                    <div className="text-center py-16 animate-fade-in">
                        ...
                        <p className="text-sm font-medium text-text-muted">Aucune alerte pour ces filtres</p>
                        <p className="text-xs text-text-muted mt-1">Essayez de modifier vos critères</p>
                    </div>
                ) : (
                    filteredAlertes.map((alerte, index) => {
                        ...
                        return (
                            <Link href={`/alertes/${alerte.id}`} key={alerte.id} className="block animate-fade-in" style={{ animationDelay: `${index * 0.05}s` }}>
                                <div className={cn(
                                    "bg-white rounded-2xl border border-border/80 overflow-hidden border-l-4 hover:shadow-lg transition-all duration-200 group",
                                    style.border
                                )}>
                                    <div className="p-4">
                                        {/* Top row */}
                                        <div className="flex items-center justify-between mb-2">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-[10px] text-text-muted">{formatRelativeTime(alerte.dateEmission)}</span>
                                            </div>
                                        </div>

                                        {/* Title */}
                                        <h3 className="text-sm font-bold text-text mt-1.5 leading-snug">
                                            ⚠️ {alerte.titre}
                                        </h3>
                                        ...
                                        <div className="flex items-center justify-between mt-3 pt-2 border-t border-gray-50">
                                            <span className="text-[11px] text-text-muted font-medium">📍 {alerte.zone}</span>
